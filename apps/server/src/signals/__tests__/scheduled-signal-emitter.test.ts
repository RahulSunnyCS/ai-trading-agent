...
async function driveSnapshots(
  ...
  for (const fields of snapshots) {
    ...
    await (
      emitter as unknown as { _handleSnapshot: (f: Record<string, string>) => Promise<void> }
...
describe('ScheduledSignalEmitter', () => {
  ...
  it('emits SCHEDULED signal on first snapshot at the configured time', async () => {
    ...
    await driveSnapshots(emitter, [makeSnapshotFields({ time: String(IST_0917_MAY19) })]);
  ...
  it('emits SCHEDULED signal only once per calendar day per underlying', async () => {
    ...
    const scheduledCalls = mockRedis.xaddCalls.filter((c) => c.fields.signal_type === 'SCHEDULED');
  ...
  it('does NOT emit SCHEDULED signal outside market hours (08:00 IST)', async () => {
    ...
    await driveSnapshots(emitter, [makeSnapshotFields({ time: String(IST_0800_MAY19) })]);
  ...
  it('emits PULLBACK signal when straddle drops >= pullbackRetracePct% from peak', async () => {
    ...
    await driveSnapshots(emitter, [makeSnapshotFields({ time: String(t3), straddleValue: '500' })]);

    const pullbackCalls = mockRedis.xaddCalls.filter((c) => c.fields.signal_type === 'PULLBACK');
  ...
  it('does NOT emit PULLBACK signal when drop is below pullbackRetracePct%', async () => {
    ...
    await driveSnapshots(emitter, [makeSnapshotFields({ time: String(t2), straddleValue: '495' })]);
  ...
  it('suppresses second PULLBACK within the dedup window (600s)', async () => {
    ...
    const t2 = IST_0917_MAY19 + 15_000;
    await driveSnapshots(emitter, [makeSnapshotFields({ time: String(t2), straddleValue: '480' })]);

    // Recovery: straddle bounces to 490.
    const t3 = t2 + 15_000;
    await driveSnapshots(emitter, [makeSnapshotFields({ time: String(t3), straddleValue: '490' })]);
    ...
    const t4 = t3 + 15_000;
    await driveSnapshots(emitter, [makeSnapshotFields({ time: String(t4), straddleValue: '470' })]);
  ...
  it('emits a second PULLBACK after the dedup window expires', async () => {
    ...
    const t3 = t2 + 15_000;
    await driveSnapshots(emitter, [makeSnapshotFields({ time: String(t3), straddleValue: '510' })]);
    ...
    await driveSnapshots(emitter, [makeSnapshotFields({ time: String(t4), straddleValue: '493' })]);
  ...
  it('start() processes stream messages and emits signals via xadd', async () => {
    ...
    await Promise.race([emitter.start(), new Promise<void>((resolve) => setTimeout(resolve, 50))]);
