...
async function getClockworkId(
  ...
  if (!clockwork) {
    throw new Error('Clockwork personality (isFrozen=true) not found — run migrations first');
  }
  return clockwork.id;
}
...
async function getMomentumPersonalities(
  ...
  Array<{ id: string; name: string; entryType: string; params: Record<string, unknown> }>
> {
  const res = await request.get('/personalities?include_inactive=true');
  expect(res.status()).toBe(200);
  const personalities = (await res.json()) as Array<{
...
async function getMutablePersonalityId(
  ...
  const candidate = personalities.find((p) => !p.isFrozen && p.entryType === 'momentum_exhaustion');
  if (!candidate) {
...
test('@critical GET /personalities returns a list of personalities', async ({ request }) => {
...
test('@functional GET /personalities returns 9 active personalities by default (Levelhead excluded)', async ({
  request,
}) => {
  ...
  const levelhead = body.find((p) => p.name.toLowerCase() === 'levelhead');
  expect(levelhead).toBeUndefined();
});

test('@functional GET /personalities?include_inactive=true returns 10 personalities', async ({
  request,
}) => {
  ...
  const response = await request.get('/personalities?include_inactive=true');
...
test('@non-blocker GET /personalities/:id returns 404 for unknown UUID', async ({ request }) => {
  ...
  const response = await request.get('/personalities/00000000-0000-0000-0000-000000000000');
...
test('@critical PUT /personalities/:id returns 403 FROZEN_VIOLATION when target is Clockwork (is_frozen=TRUE)', async ({
  request,
}) => {
...
test('@critical PUT /personalities/:id returns 409 COMPARISON_INTEGRITY_VIOLATION when min_probability drift > 8pp', async ({
  request,
}) => {
...
test('@functional PUT /personalities/:id validates param ranges and returns 400 for out-of-range values', async ({
  request,
}) => {
  ...
  const belowFloor = await request.put(`/personalities/${mutableId}`, {
    data: { params: { min_probability: 0.3 } },
  });
...
test('@functional PUT /personalities/:id writes audit log entry on successful change', async ({
  request,
}) => {
  ...
  const newMinProb = Number.parseFloat(
    Math.max(0.4, Math.min(0.9, originalMinProb + 0.01)).toFixed(2),
  );
...
test('@critical GET /personalities/:id/performance excludes pre-M2 NULL personality_id rows', async ({
  request,
}) => {
  ...
  const personalities = (await (await request.get('/personalities')).json()) as Array<{
    id: string;
    name: string;
  }>;
  ...
  const response = await request.get(`/personalities/${targetId}/performance`);
...
test('@critical GET /personalities/:id/performance returns personality-scoped stats only (does not aggregate across all personalities)', async ({
  request,
}) => {
