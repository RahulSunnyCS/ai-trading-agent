 ...
 */
async function fetchInstrument(symbol: string, nowMs: number): Promise<MacroDataPoint | null> {
  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
    ...
    try {
      ...
      if (attempt < MAX_ATTEMPTS) {
        ...
        console.warn(
          `[GlobalMacroFeed] ${symbol} fetch attempt ${attempt} failed: ${msg} — retrying`,
        );
        continue;
      }
      console.warn(
        `[GlobalMacroFeed] ${symbol} fetch failed after ${MAX_ATTEMPTS} attempts: ${msg}`,
      );
      return null;
    }
...
export class GlobalMacroFeed {
  ...
  constructor(
    ...
    this._intervalMs = Number.parseInt(process.env.MACRO_POLL_INTERVAL_MS ?? '300000', 10);
    ...
    if (Number.isNaN(this._intervalMs) || this._intervalMs < 10_000) {
      console.warn(
  ...
  async _doPoll(): Promise<void> {
    ...
    for (const settled of results) {
      if (settled.status === 'rejected') {
        ...
        console.warn(`[GlobalMacroFeed] unexpected rejection in poll: ${settled.reason}`);
        continue;
      }
      ...
      try {
        await this.redis.set(`macro:${inst.key}`, JSON.stringify(dp), 'EX', REDIS_TTL_SECONDS);
      } catch (err) {
        const msg = err instanceof Error ? err.message : String(err);
