...
async function checkComparisonIntegrity(
  ...
  for (const row of result.rows) {
    const effectiveParams =
      row.id === updatedPersonalityId ? proposedParams : row.params;
  ...
  if (divergencePp > 8) {
    ...
    const outlier = probabilities.reduce((worst, current) =>
      Math.abs(current.prob - medianProb) > Math.abs(worst.prob - medianProb)
        ? current
        : worst,
    );
...
export const personalitiesRoutes: FastifyPluginAsync<PersonalitiesRoutesOptions> = async (
  ...
  fastify.get(
    ...
    async (request, _reply) => {
      ...
      const result = await opts.db.query<PersonalityRow>(sql, params.length > 0 ? params : undefined);
  ...
  fastify.put(
    ...
    async (request, reply) => {
      ...
      if (!hasIsActive && !hasParams) {
        return reply
          .code(400)
          .send({ error: "EMPTY_UPDATE", message: "Request body must include is_active or params" });
      }
