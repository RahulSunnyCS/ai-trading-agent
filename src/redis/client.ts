...
// maxRetriesPerRequest: 3 — retry transient errors without crashing the process.
...
// Checked by every active streamConsume loop.
// A single boolean is sufficient because Node.js/Bun is single-threaded;
// no lock needed.
let shuttingDown = false;

// ---------------------------------------------------------------------------
// streamPublish — write one message to a Redis Stream
// ---------------------------------------------------------------------------
 ...
 *
 * Accepts either a StreamName (typed const) or a plain string for backward
 * compatibility with callers that pass arbitrary stream names.
 *
...
// ---------------------------------------------------------------------------
// streamRead — read messages from a Redis Stream
// ---------------------------------------------------------------------------
 ...
 * to this consumer so they can be reprocessed.
 */
