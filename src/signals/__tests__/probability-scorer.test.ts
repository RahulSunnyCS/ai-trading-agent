...
describe('India VIX adjustment', () => {
  ...
  it('VIX=20 → india_vix adjustment = -0.025', () => {
    ...
    expect(r4(result.adjustmentBreakdown.india_vix ?? Number.NaN)).toBe(-0.025);
  });
...
describe('confidenceTier derivation', () => {
  it('adjustedProbability >= 0.70 → HIGH', () => {
    ...
    expect(result.adjustedProbability).toBeGreaterThanOrEqual(0.7);
  ...
  it('adjustedProbability >= 0.50 and < 0.70 → MEDIUM', () => {
    ...
    expect(result.adjustedProbability).toBeGreaterThanOrEqual(0.5);
    expect(result.adjustedProbability).toBeLessThan(0.7);
  ...
  it('adjustedProbability < 0.50 → LOW', () => {
    ...
    expect(result.adjustedProbability).toBeLessThan(0.5);
