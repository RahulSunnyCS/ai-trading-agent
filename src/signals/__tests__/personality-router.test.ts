...
function makeSignalFields(overrides: Partial<Record<string, string>> = {}): string[] {
  ...
  return Object.entries(defaults).flatMap(([k, v]) => [k, v]);
}
...
describe("PersonalityRouter", () => {
  ...
  it("calls openTrade twice when signal passes filters for 2 active personalities", async () => {
    ...
    const personality2 = makePersonality({ id: "pers-002", name: "blitz", managementStyle: "cut_reenter" });
  ...
  it("logs open trades during startup reconciliation", async () => {
    ...
    expect(
      infoCalls.some((msg) =>
        msg.includes("Startup reconciliation") && msg.includes("2 open trade(s)"),
      ),
    ).toBe(true);
    ...
    expect(
      infoCalls.some(
        (msg) => msg.includes("trade-002") && msg.includes("no personality_id"),
      ),
    ).toBe(true);
  ...
  it("calls fetchDailyState once per active personality in parallel", async () => {
    ...
    vi.mocked(runPersonalityFilter).mockReturnValue({ pass: false, stage: 1, reason: "PERSONALITY_INACTIVE" });
