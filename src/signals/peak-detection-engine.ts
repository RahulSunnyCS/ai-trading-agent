...
import { type MacroContext, getMacroContext } from '../ingestion/global-macro-feed.js';
...
export class PeakDetectionEngine {
  ...
  async _handleSnapshot(fields: Record<string, string>): Promise<void> {
    ...
    const expansionMet = expansionPct !== null && expansionPct >= this._config.minExpansionPct;
    const accelerationMet =
      acceleration !== null && acceleration < this._config.accelerationThreshold;
    const rocDeclineMet = state.rocDeclineStreak >= this._config.rocDeclineCandles;
