...
export class ScheduledSignalEmitter {
  ...
  private async _handleSnapshot(fields: Record<string, string>): Promise<void> {
    ...
    const straddleValue = straddleValueStr !== "" ? Number.parseFloat(straddleValueStr) : NaN;
    ...
    const snapshotTimeMs = fields.time !== undefined ? Number.parseInt(fields.time, 10) : this.clock.now();
    ...
    if (state.peakStraddleValue !== null) {
      ...
      const dropPct =
        ((state.peakStraddleValue - straddleValue) / state.peakStraddleValue) * 100;
...
export function buildConfigFromEnv(): FallbackSignalConfig {
  ...
  const pullbackRetracePct = Number.isFinite(rawRetracePct) && rawRetracePct > 0 ? rawRetracePct : 3;

  const rawLookback = Number.parseInt(process.env.PULLBACK_LOOKBACK_CANDLES ?? "8", 10);
  const pullbackLookbackCandles =
    Number.isFinite(rawLookback) && rawLookback > 0 ? rawLookback : 8;

  const rawDedup = Number.parseInt(process.env.PULLBACK_DEDUP_WINDOW_SECS ?? "600", 10);
  const pullbackDedupWindowSecs =
    Number.isFinite(rawDedup) && rawDedup > 0 ? rawDedup : 600;
