  ...
  private async _handleSignal(_msgId: string, flatFields: string[]): Promise<void> {
    ...
    const filterResults = await Promise.all(
      personalities.map((p, i) =>
        ...
        runPersonalityFilter(straddleSignal, p, dailyStates[i]!, nowMs),
      ),
    );
    ...
    if (passingIntents.length > 4) {
      console.warn(
        `[personality-router] ${passingIntents.length} personalities passed filters simultaneously ` +
          `(max 4 recommended) — T-31 portfolio-risk check not yet implemented`,
      );
    }
