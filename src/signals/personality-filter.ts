...
export async function fetchDailyState(
  ...
  return {
    tradeCount: Number.parseInt(row?.today_trade_count ?? '0', 10),
    netPnl: String(row?.today_net_pnl ?? '0'),
    openPositions: Number.parseInt(openRow?.open_legs ?? '0', 10),
  };
}
...
export function runPersonalityFilter(
  ...
  if (personality.entryType === 'momentum_exhaustion' && signal.signalType === 'SCHEDULED') {
    return {
      pass: false,
      stage: 1,
      reason:
  ...
  const maxDailyTrades =
    (personality.params.max_daily_trades as number | undefined) ?? Number.POSITIVE_INFINITY;
  ...
  if (maxDailyLoss !== undefined) {
    ...
    if (Number.parseFloat(dailyState.netPnl) <= -maxDailyLoss) {
  ...
  if (requireProfitGate === true) {
    ...
    if (profitGateAmount !== undefined) {
      ...
      if (Number.parseFloat(dailyState.netPnl) >= profitGateAmount) {
...
export function checkComparisonIntegrity(
  ...
  // probs is non-empty (candidates.length >= 2 guard above), so the sentinels
  // are always overwritten by the loop.
  let minProb = Number.POSITIVE_INFINITY;
  ...
  const seed = probs[0];
  if (seed === undefined) return { valid: true };
  let offender = seed;
