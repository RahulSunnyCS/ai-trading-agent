...
test('All three wired tabs show an error or unavailable state when the backend is completely unreachable @functional', async ({
  page,
}) => {
...
test('PaymentTestModeBanner remains visible on all four tabs @non-blocker', async ({ page }) => {
  await installStandardMocks(page);
  await page.goto('/');
...
test('Tab buttons are keyboard-focusable and activatable via Enter @non-blocker', async ({
  page,
}) => {
  await installStandardMocks(page);
  await page.goto('/');
