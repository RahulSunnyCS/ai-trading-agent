...
test(
  'NUMERIC string fields (gross_pnl, net_pnl, straddle_at_entry) render as formatted numbers, not raw strings @critical',
  async ({ page }) => {
    const trades = [
      makeTrade({
        id: 'trade-1',
    ...
    const row = page.getByRole('row').filter({ hasText: /Closed/i }).first();
...
test(
  'Negative net_pnl is colored red and positive net_pnl is colored green @critical',
  async ({ page }) => {
    const trades = [
      makeTrade({
        id: 'trade-neg',
...
test(
  'Open trades with null net_pnl show an em dash placeholder — never NaN or undefined @critical',
  async ({ page }) => {
    const trades = [
      makeTrade({
        id: 'trade-open',
...
test(
  'When /api/trades returns an empty array TradesView shows a clear "No trades yet" empty state @critical',
  async ({ page }) => {
...
test(
  'When /api/trades returns HTTP 500 TradesView shows an error notice and does not crash @critical',
  async ({ page }) => {
...
test(
  'Entry times are displayed in IST — 04:00 UTC renders as 09:30 IST @functional',
  async ({ page }) => {
    const trades = [
      makeTrade({
        id: 'trade-ist',
...
test(
  'Status badges render for open and closed trade status values @functional',
  async ({ page }) => {
    const trades = [
...
test(
  'When /api/trades returns HTTP 404 TradesView shows an error notice rather than an empty table @functional',
  async ({ page }) => {
    await page.route('**/api/trades', (route) => {
...
test(
  'Exit reason is shown for closed trades and a dash for open trades @non-blocker',
  async ({ page }) => {
    const trades = [
      makeTrade({
        id: 'trade-closed',
...
test(
  'Straddle-at-entry column shows a formatted decimal number, not scientific notation @non-blocker',
  async ({ page }) => {
    const trades = [
      makeTrade({
        id: 'trade-1',
