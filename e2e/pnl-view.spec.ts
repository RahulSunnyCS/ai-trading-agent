 ...
 */
async function openPnlTab(
  page: Page,
  tradesPayload: unknown = { data: [] },
): Promise<void> {
...
test(
  'Total net P&L is the correct arithmetic sum — not string concatenation @critical',
  async ({ page }) => {
    const trades = [
      makeTrade({
        id: 'trade-1',
...
test(
  'Open trades with null net_pnl are excluded from the total P&L sum @critical',
  async ({ page }) => {
    const trades = [
      makeTrade({
        id: 'trade-closed',
...
test(
  'When /api/trades returns HTTP 500 PnlView shows an error notice, not a zeroed-out P&L dashboard @critical',
  async ({ page }) => {
...
test(
  "Today's P&L uses IST date boundaries — a trade at 23:59 IST is counted in the correct IST day @critical",
  async ({ page }) => {
...
test(
  'Win rate is computed as closed-wins / total-closed — open trades excluded from denominator @functional',
  async ({ page }) => {
...
test(
  'Cumulative P&L chart renders without crash when there are only open trades @functional',
  async ({ page }) => {
    const pageErrors: string[] = [];
    page.on('pageerror', (err) => { pageErrors.push(err.message); });

    const trades = [
      makeTrade({ id: 'open-only', status: 'open', exit_time: null, net_pnl: null }),
    ];
...
test(
  'When /api/trades returns an empty array PnlView shows a no-closed-trades empty state @functional',
  async ({ page }) => {
    const pageErrors: string[] = [];
    page.on('pageerror', (err) => { pageErrors.push(err.message); });
...
test(
  'Open and closed position counts are displayed separately and accurately @functional',
  async ({ page }) => {
...
test(
  'Total net P&L is colored green for positive values and red for negative @non-blocker',
  async ({ page }) => {
