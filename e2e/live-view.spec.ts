...
test(
  'The tick chart area labels the feed as synthetic/dev — not real straddle data @critical',
  async ({ page }) => {
...
test(
  'When /api/straddle/latest returns { data: null } the UI shows a graceful "not yet connected" notice with no numeric value @critical',
  async ({ page }) => {
    ...
    const straddleCard = page
      .locator('div.rounded-lg')
      .filter({ hasText: 'NIFTY Straddle Value' });
...
test(
  'LiveView shows a connection-status pill in Connecting state when no WS server is reachable @critical',
  async ({ page }) => {
    await openLiveTab(page);
    ...
    const isValid = validStates.some((s) =>
      ariaLabel!.toLowerCase().includes(s.toLowerCase()),
    );
...
test(
  'Connection pill transitions to Disconnected or reconnecting state when the WebSocket cannot connect @critical',
  async ({ page }) => {
    await openLiveTab(page);
...
test(
  'Switching away from LiveView to another tab does not leave stale console errors @critical',
  async ({ page }) => {
    ...
    const reactUnmountWarnings = consoleErrors.filter((e) =>
      e.includes('unmounted component') || e.includes("Can't perform a React state update"),
    );
...
test(
  'Switching away from LiveView and back does not crash or show visual corruption @non-blocker',
  async ({ page }) => {
...
test(
  'Connection status pill has an accessible aria-label with the current status @non-blocker',
  async ({ page }) => {
    await openLiveTab(page);
